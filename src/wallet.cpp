...
// dominate: optional setting to unlock wallet for block minting only;
...
// dominate: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // dominate: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // dominate: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // dominate: not avatar mode
                    {
...
// dominate: create coin stake transaction
...
// dominate: check 'spent' consistency between wallet and txindex
// dominate: fix wallet spent state according to txindex
...
// dominate: disable transaction (only for coinstake)
